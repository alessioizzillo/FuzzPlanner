...
uint32_t ip_string_to_int(char *ip_string) {
    uint32_t ip_int = 0;
    int octets[4];
    
    sscanf(ip_string, "%d.%d.%d.%d", &octets[0], &octets[1], &octets[2], &octets[3]);
    
    for (int i = 0; i < 4; i++) {
        ip_int += octets[i] << (24 - (8 * i));
    }
...
int send_testcase(const char* filename) {
  ...
  size_t bytes_read = 0;
  long file_size = 0, old_file_size = 0;
  struct stat st;
  int while_break = 0;
  ...
  while (1) {
    ...
    size_t n = bytes_read < sizeof(chunk) ? bytes_read : sizeof(chunk);
    if (send(sockfd, chunk, n, 0) < 0) {
      perror("send");
      exit(1);
    }

    struct stat st;
    while (file_size >= old_file_size){
      if (stat(filename, &st) == 0) {
        file_size = st.st_size;
      }
      else{
...
void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  // Temporary variables for the replay mode
  u64 tmp_start_time = 0;
  u64 tmp_last_update = 0;
  u64 tmp_execs_done = 0;
  u32 tmp_paths_total = 0;
  u32 tmp_paths_favored = 0;
  u64 tmp_last_phase = 0;
...
static double get_runnable_processes(void) {
  ...
  /* On Linux, /proc_host/stat is probably the best way; load averages are
     computed in funny ways and sometimes don't reflect extremely short-lived
     processes well. */

  FILE* f = fopen("/proc_host/stat", "r");
  u8 tmp[1024];
  u32 val = 0;
...
static void show_stats(void) {
  ...
  if(cur_ms > start_time+(fuzzing_timeout*1000))
    stop_soon = 2;
