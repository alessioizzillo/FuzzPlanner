...
void extractFDs(const char* input, int* fd1, int* fd2) {
    ...
    if (firstUnderscore == NULL) {
        printf("Underscore not found\n");
        return;
    }

    const char* secondUnderscore = strchr(firstUnderscore + 1, '_');
    if (secondUnderscore == NULL) {
        printf("Second underscore not found\n");
        return;
    }

    *fd1 = atoi(firstUnderscore + 1);
    *fd2 = atoi(secondUnderscore + 1);
}


char* extractParams(const char* str) {
    if(!str)
        return NULL;
    ...
    char* path = malloc((pathLength + 1) * sizeof(char));
    if (path == NULL) {
        return NULL;
    }

    strncpy(path, start, pathLength);
    path[pathLength] = '\0';
...
static void fuzz_mem_write(DECAF_Callback_Params *dcp)
{
    ...
    if(afl_user_fork == 1)
    {
        ...
        // IOT-AFL fix for "cross page"
        if ((virt_addr & 0xfff) + dl > 0x1000 && (virt_addr & 0xfff) + dl < 0x2000)
        {
            ...
            phys_addr2 = DECAF_get_phys_addr(cpu, virt_addr+0x1000);
        } 
// memory consistence
#ifdef SNAPSHOT_SYNC
        if(in_httpd && (virt_addr < kernel_base))
        {
            ...
            if (phys_addr2){
                ifexist = if_physical_exist(phys_addr2 & 0xfffff000);
                if(ifexist)
                {
                    return;
                }

                add_physical_page(phys_addr2 & 0xfffff000);
            }
        }
#endif
        ...
        if (phys_addr2)
            store_page((virt_addr+0x1000) & 0xfffff000, page_host_addr+0x1000, in_httpd); // I assume the page_host_addr of virt_addr+0x1000 is page_host_addr+0x1000
#ifdef CAL_TIME_ext
...
STATE *get_state(int pid)
{
    for (STATE *curr = state_head; curr != NULL; curr = curr->next)
    {
        int tmp_pid = curr->pid;
        if (tmp_pid == pid)
        {
            return curr;
        }
    ...
    new_state->pid = pid;
    new_state->par_pid = 0;
    memset(new_state->procname, 0, 64);
    new_state->pc = 0;
    new_state->sp = 0;
    new_state->a0 = 0;
    new_state->a1 = 0;
    ...
    new_state->a3 = 0;
    new_state->a4 = 0;
    new_state->into_syscall = 0;
    if (state_head == NULL)
    {
        state_head = new_state;
        new_state->next = NULL;
    }
    else
    {
        STATE *tmp = state_head;
        state_head = new_state;
        new_state->next = tmp;
    }
    return new_state;
...
STATE *find_state_from_pc_and_stack(target_ulong pc, target_ulong sp)
{
    for (STATE *curr = state_head; curr != NULL; curr = curr->next)
    {
        target_ulong tmp_pc = curr->pc;
        target_ulong tmp_sp = curr->sp;
#ifdef TARGET_MIPS
        if (tmp_pc + 4 == pc && tmp_sp == sp)
#elif defined(TARGET_ARM)
        if (tmp_pc == pc && tmp_sp == sp)
#endif
        {
            return curr;
        }
...
int update_state(STATE *state, int pid, int par_pid, const char procname[], int syscall_nr,
                 target_ulong pc, target_ulong sp, target_ulong a0, target_ulong a1, target_ulong a2,
                 target_ulong a3, target_ulong a4)
{
    state->pid = pid;
    state->par_pid = par_pid;
    memset(state->procname, 0, 64);
    if (procname)
        memcpy(state->procname, procname, strlen(procname));
    state->into_syscall = syscall_nr;
    state->pc = pc;
    state->sp = sp;
    state->a0 = a0;
    state->a1 = a1;
...
int determine_if_network_recv(int program_id, CPUState *cpu)
{
    ...
    if (!afl_user_fork && handle_recv == 2)
        return 1;
        ...
        if (sys_call_num == read_syscall || sys_call_num == recv_syscall || sys_call_num == recvfrom_syscall)
        {
            ...
            if(find_pgd(pgd)) 
            {
                if (debug){
                    ...
                    fprintf(fd, "\ndetermine_if_network_recv:%d,%d\n\n",arg_0, accept_fd);
                    fclose(fd);
                }

                DECAF_printf("determine_if_network_recv:%d,%d\n",arg_0, accept_fd);
                handle_recv = 1;
                storeCPUShState(&pre_fork_cpustate, env);
                // return 1;//goto skip_to_pos;
            }
        }
    }
...
void printHexBuffer(FILE* file, const unsigned char* buffer, size_t size) {
    for (size_t i = 0; i < size; i++) {
        fprintf(file, "%02X ", buffer[i]);
    }
}

char* visualizeUTF8(const char *data, size_t length) {
    char *output = (char*)malloc((length * 12) + 1);
    if (output == NULL) {
        ...
        if (byte < 32 || byte > 126 || byte == '"') {
            if (byte >= 0xC0 && i + 1 < length && (data[i + 1] & 0xC0) == 0x80) {
                ...
                if (code_point >= 0xD800 && code_point <= 0xDFFF) {
                    snprintf(output + outputIndex, 13, "\\u%04X\\u%04X", (code_point >> 10) + 0xD7C0, (code_point & 0x3FF) + 0xDC00);
                    outputIndex += 12;
                } else if (code_point > 0xFFFF) {
                    snprintf(output + outputIndex, 13, "\\u%04X\\u%04X", (code_point >> 10) + 0xD7C0, (code_point & 0x3FF) + 0xDC00);
                    outputIndex += 12;
                } else {
                    snprintf(output + outputIndex, 7, "\\u%04X", code_point);
                    outputIndex += 6;
                }
                i++;
            } else {
                snprintf(output + outputIndex, 7, "\\u%04X", byte);
                outputIndex += 6;
            }
        } else {
...
int cpu_exec(CPUState *cpu)
{
    ...
    if(afl_user_fork == 0 && cpu->exception_index == exception_num ) // syscall exit;
    {
...
#ifdef SHOW_SYSCALL  
            if (debug){
                FILE * fffp = fopen("debug/before_syscall_trace", "a+");
        ...
        if (start_fork_pc == 0)
        {
            ...
            target_ulong a4 = 0;
            DECAF_read_ptr(cpu, sp + 16, &a4);
#else
            target_ulong a4 = env->active_tc.gpr[8];
#endif
            ...
            if (ret == -1)
                sprintf(procname, "<kernel>");

            STATE *state = get_state(pid);

#ifdef TARGET_ARM
            if (state->into_syscall == 0 || state->into_syscall == 11)
#elif defined(TARGET_MIPS)
            if (state->into_syscall == 0 || state->into_syscall == 4011)
#endif
            {
                ...
                update_state(state, pid, par_pid, procname, syscall_nr, pc, sp, a0, a1, a2, a3, a4);
            }
        }
    }
...
skip_to_pos:
            ...
            if (fuzz)
            {
...
#endif
                {
                    ...
                    FILE * fffp;
                    if (debug){
                        fffp = fopen("debug/before_syscall_trace", "a+");
                    ...
                    if(new_pgd == target_pgd){ //user_stack_count // || get_current_pc()  == 0xb960 + tmp_libuclibc_addr
                        if (debug){
                            FILE *fd= fopen("debug/pre_fork_syscalls.log","a+");
                            fprintf(fd, "SYSTEM-MODE (syscall tracing): SYSCALL %d (pc: %lx)", into_syscall, pc);
                ...
                {
                    target_ulong pgd =DECAF_getPGD(cpu);
                    if(debug && find_pgd(pgd) && pc < 0x10000000)
                    {
                        FILE * fffp = fopen("debug/before_syscall_trace", "a+");
                        fprintf(fffp, "pc: %x\n", pc);
                        fclose(fffp);
            ...
            if (start_fork_pc == 0)
            {
                ...
                if (afl_user_fork == 0)
                {
                    if (pc == do_coredump_addr)
                    {
                        ...
                        if (debug)
                        {
                            FILE *fd = fopen("debug/full_crashes.log", "a+");
                            fprintf(fd, "%lu,%d,%s\n", milliseconds, pid, procname);
                            fclose(fd);
                        }
                    }

                    STATE *state = find_state_from_pc_and_stack(pc, stack);
                    if (state)
                    {
                        CPUArchState *env = cpu->env_ptr;
                        target_ulong sp = state->sp;
                        target_ulong a0 = state->a0;
                        target_ulong a1 = state->a1;
                        target_ulong a2 = state->a2;
                        target_ulong a3 = state->a3;
                        target_ulong a4 = state->a4;
#ifdef TARGET_MIPS
                        target_ulong err = env->active_tc.gpr[7];
                        target_ulong ret_value_0 = env->active_tc.gpr[2];
                        target_ulong ret_value_1 = env->active_tc.gpr[3];
#elif defined(TARGET_ARM)
                        target_ulong err = 0;                    //???
                        ...
                        if (debug)
                        {
                            ...
                            if (state->into_syscall == 4005) // open
                            {
                                ...
                                memset(buf, 0, 100);
                                memset(path, 0, 100);
                                tlb_fill(cpu, a0, MMU_DATA_LOAD, cpu_mmu_index(env, true), 0);
                                DECAF_read_mem(cpu, a0, 100, buf);
                                if (buf[strlen(buf) - 1] == '\n')
                                ...
                                if (execution_mode && !strcmp(state->procname, target_exec) && !target_replay_fd)
                                {
                                    ...
                                    {
                                        char *target_path = extractParams(target_channel);
                                        char tmp_path[100] = {0};
                            ...
                            else if (state->into_syscall == 4169 || state->into_syscall == 4170) // bind & connect
                            {
                                ...
                                DECAF_read_mem(cpu, a1, sizeof(unsigned short), &family);
                                if (family == 1 || family == 256)
                                { // AF_UNIX
                                    struct sockaddr_un sockaddr;
                                    DECAF_read_mem(cpu, a1, sizeof(struct sockaddr_un), &sockaddr);
                            ...
                            else if (state->into_syscall == 4180 || state->into_syscall == 4176) // sendto & recvfrom
                            {
                                unsigned short family;
                                tlb_fill(cpu, a4, MMU_DATA_LOAD, cpu_mmu_index(env, true), 0);
                                DECAF_read_mem(cpu, a4, sizeof(unsigned short), &family);
                                if (family == 1 || family == 256)
                                { // AF_UNIX
                                    struct sockaddr_un sockaddr;
                            ...
                            else if (state->into_syscall == 4177 || state->into_syscall == 4179) // recvmsg & sendmsg
                            {
                                target_ulong msg_name_addr;
                                tlb_fill(cpu, a1, MMU_DATA_LOAD, cpu_mmu_index(env, true), 0);
                                DECAF_read_ptr(cpu, a1, &msg_name_addr);
                                unsigned short family;
                                DECAF_read_mem(cpu, msg_name_addr, sizeof(unsigned short), &family);
                                if (family == 1 || family == 256)
                                { // AF_UNIX
                                    struct sockaddr_un sockaddr;
                                ...
                                if (execution_mode && target_replay_syscall == state->into_syscall)
                                {
                                    ...
                                    tlb_fill(cpu, a1+8, MMU_DATA_LOAD, cpu_mmu_index(env, true), 0);
