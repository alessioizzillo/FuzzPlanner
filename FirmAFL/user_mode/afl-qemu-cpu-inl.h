...
//SHMAT
    ...
    FILE* file = fopen(extern_struct->aflFile, "r+");
    if (file == NULL) {
      perror("Failed to open file");
      return 1;
    }

    char *content = (char *) malloc(extern_struct->buf_read_index);
    fread(content, 1, extern_struct->buf_read_index, file);

    // Set the file position to the beginning and truncate the file
    fseek(file, 0, SEEK_SET);
    if (truncate(extern_struct->aflFile, 0) != 0) {
    ...
    fwrite(content, 1, extern_struct->buf_read_index, file);
