    ...
    else if(strcmp(feed_type, "FEED_HTTP") == 0)
    {
        ...
        */
        if (debug){
            ...
            fprintf(fp, "get_input()\n");
            fclose(fp);
        }
        total_len = getWork(&recv_buf);
        if (debug){
            FILE *fp= fopen("debug/post_fork_syscalls.log","a+");
            fprintf(fp, "Got testcase by get_work()\n");
            fclose(fp);
        }
    ...
    *network_handle = 1;
    if(strcmp(feed_type,"FEED_HTTP") == 0)
    {
        ...
        if((syscall_num == 175  || syscall_num == 3 || syscall_num == 176) && (a0 == accept_fd) && feed_input_times == 0)
        {
            ...
            //printf("recv :%d,%d,%d, %s\n", accept_fd, len,extern_struct->buf_read_index, recv_buf);
            int rest_len = total_len - extern_struct->buf_read_index;
            //printf("hook flag:%x, %x\n", MSG_PEEK, flag);
            if(rest_len > len)
            {
                ret = len;
                memcpy(buf, recv_buf + extern_struct->buf_read_index, ret);
            }
            else
            {
                ret  = rest_len;
...
int main(int argc, char **argv, char **envp)
{
    ...
    int shmid = shmget(IPC_PRIVATE, sizeof(SharedVariables), IPC_CREAT | 0666);
    if (shmid == -1) {
        perror("shmget");
        exit(1);
    }

    extern_struct = (SharedVariables *)shmat(shmid, NULL, 0);
    if (extern_struct == (SharedVariables *)-1) {
        perror("shmat");
        exit(1);
    }
...
#ifdef FEED_INPUT
    extern_struct->aflFile =  target_argv[1];
