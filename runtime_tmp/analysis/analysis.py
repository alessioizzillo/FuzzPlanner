...
if __name__ == "__main__":
    ...
    status = ""
    with open("backend_runtime/emulation/%s/status"%firmware_name, 'r') as file:
        status=file.read().strip()
        
    with open("backend_runtime/emulation/%s/status"%firmware_name, 'w') as file:    
        if (status == "stopped_analyzing"):
            file.write("stopped")
        else:
            file.write("paused")
