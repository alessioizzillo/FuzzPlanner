...
import { useOptimizedSelectProgress } from '@/hooks/useOptimizedPolling'

export default function SelectProgress({ containerName }) {
  const { progress, isLoading: loading, error } = useOptimizedSelectProgress(containerName)
  ...
  const getPhaseColor = (phase) => {
    switch (phase) {
      ...
      case 'replaying': return 'text-blue-500'
        ...
        <div className="w-full bg-gray-200 rounded-full h-1 mt-1">
          <div
            className={`h-1 rounded-full transition-all duration-300 ${
              ...
              progress.phase === 'replaying' ? 'bg-blue-500' :
